...
def plot_kpca_kernels(df_results, filename="kpca_kernels_plot.png"):
    # Now iterating over Kernels instead of distributions
    ...
    for i, kernel in enumerate(kernels):
        ax = axes[i]
        group = df_results[df_results["Kernel"] == kernel]
        ...
        ax.set_xscale("log")
        # Updated Title
        ax.set_title(f"{kernel} (Gaussian Data)", fontweight="bold")
    ...
    print(f"[+] Saved exact KPCA kernels plot to '{filename}'")

if __name__ == "__main__":
    data_filename = "outputs/kpca_kernels_results.csv"
    plot_filename = "outputs/kpca_kernels_plot.png"
    
    try:
        # Load and plot
        df_loaded = pd.read_csv(data_filename)
        plot_kpca_kernels(df_loaded, filename=plot_filename)
