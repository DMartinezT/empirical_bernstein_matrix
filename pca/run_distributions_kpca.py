...
if __name__ == "__main__":
    ...
    # 2. Run the heavy simulation
    print("Running KPCA simulations...")
    df_results = simulate_kpca_distributions(n_values, d=d, trials=trials)
    
    # 3. Print results to console
    for dist, group in df_results.groupby("Distribution"):
        print(f"\n--- {dist} ---")
        display_cols = [c for c in df_results.columns if c != "Distribution"]
